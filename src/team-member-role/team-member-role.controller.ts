...
export class TeamMemberRoleController {
  ...
  @Post(':teamId/assign')
  async assignRoles(
    @Param('teamId', ParseIntPipe) teamId: number, // convert string to number
    @Body('users') users: { userId: number; role: string }[],
  ): Promise<any> {
  ...
  @Get(':teamId')
  async getTeamRoles(@Param('teamId', ParseIntPipe) teamId: number): Promise<any> {
