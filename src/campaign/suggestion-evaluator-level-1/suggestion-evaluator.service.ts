...
export class SuggestionEvaluatorService {
  ...
  async evaluateWithLLM(input: EvaluateSuggestionDto): Promise<SuggestionResultDto> {
    const { strategy, role, industry, objective, keyResult } = input;

    if (!strategy || !role || !industry || !objective || !keyResult) {
      throw new Error('All fields are required: strategy, role, industry, objective, keyResult');
    }

    const prompt = predictSuggestionPrompt(strategy, role, industry, objective, keyResult);
